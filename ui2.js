$(document).ready(function($) {
		...
		*/
		...
		$("body").on('dblclick', ".ui.card", function(){
			...
			$(this).find('.price_top').delay(250).fadeOut(300);
		});
		...
		$("body").on('mouseenter', "article", function(){
			...
			/*
			$(this).find('.price_top').delay(250).fadeIn(340).transition('pulse','1000ms');

			
			setTimeout(function(){
			$(this).find('.price_top').transition('swing','1000ms');
			},1000);
			*/
		...
		*/
		...
		$("body").on('mouseleave', ".white-panel", function(){ //MOUSE-OUT KAGAK BISA CUYY
			...
			//$(this).find('.price_top').fadeOut(150);
		})
...
function load_more() {
	...
	// check to see if user want append AND FILTER
